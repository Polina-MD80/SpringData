...
public class BookServiceImpl implements BookService {
    ...
    @Override
    public void getAllBooksAfterYearByName(int year) {
        bookRepository.findAllByReleaseDateAfter(LocalDate.of(year, 12, 31))
                .stream()
                .map(Book::getTitle)
    ...
    @Override
    public void getAllAuthorsWithBooksBefore(int year) {
        List<Book> books = bookRepository.findBooksByReleaseDateBefore(LocalDate.of(1990, 1, 1));
        books.stream()
    ...
    @Override
    public void getAllBooksAsTitleReleaseAndCopiesFrom(String firstName, String lastName) {
        bookRepository.findBooksByAuthor_FirstNameAndAuthor_LastNameOrderByReleaseDateDescTitle(firstName, lastName)
                .stream().map(book -> String.format("%s Release date - %s -> %d",
    ...
    private Book createBook(String row) {
        ...
        String title = Arrays.stream(row.split("\\s+")).skip(5).collect(Collectors.joining(" "));
